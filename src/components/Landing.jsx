...
export default function Landing({ onEnter }) {
  ...
  const FEATURES = [
    ...
    { icon: '🤖', title: t('YZ rehber ve asistan', 'AI coach and assistant'), desc: t('Her adımda probleminize özgü öneri taslakları, karar matrisi ön doldurma, pre-mortem senaryoları ve serbest soru-cevap. Öneriler hipotezdir — doğrulama rozetiyle gelir, yazdıklarınız asla ezilmez.', 'Step-specific suggestion drafts, decision-matrix pre-fill, pre-mortem scenarios and free Q&A. Suggestions are hypotheses — they carry a verification badge, and your own text is never overwritten.') },
    ...
    { icon: '🕸', title: t('Vaka haritası ve raporlar', 'Case map and reports'), desc: t('Verinizden türeyen bağlantı haritası kopuk halkaları gösterir; tek tıkla A3 tek sayfa özeti, tam rapor, yönetici özeti ve salt-okunur paylaşım linki.', 'A connection map derived from your data reveals broken links; one click gives the A3 one-pager, the full report, an executive summary and a read-only share link.') },
    { icon: '🔒', title: t('Verileriniz sizde kalır', 'Your data stays with you'), desc: t('Tüm girdiler yalnızca bu tarayıcıda saklanır; sunucuya gönderilmez. Türkçe/İngilizce çift dil, açık/koyu tema, JSON yedekleme.', 'All input is stored only in this browser; nothing is sent to a server. Turkish/English bilingual, light/dark theme, JSON backup.') }
  ];
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '28px 24px 60px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '26px 0 30px' }}>
          ...
          <p style={{ maxWidth: 640, margin: '14px auto 0', font: '14.5px/1.7 Helvetica,Arial,sans-serif', color: 'var(--ink-3)' }}>
            {t('ProblemLab, bir iş problemini — hangi alanda olursa olsun — kanıtlanmış problem çözme metodolojisiyle uçtan uca çözdüren, YZ destekli bir çalışma aracıdır. Varsayımları veriden ayırır, kök nedeni doğrulatır, kararınızı sınar ve sonucu KPI ile takip ettirir.',
              'ProblemLab is an AI-assisted workbench that takes a business problem — in any domain — end to end through a proven problem-solving methodology. It separates assumptions from data, makes you verify the root cause, stress-tests your decision, and tracks the outcome against the KPI.')}
          </p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', marginTop: 22 }}>
            ...
            >{t('Önce örnek çalışmayı incele', 'Explore the example case first')}</HButton>
          </div>
        </div>
