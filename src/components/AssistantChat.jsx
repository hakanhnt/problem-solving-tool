...
import { agentTitlesFor, agentIntrosFor } from '../lib/defaults.js';
import { HButton, Spinner } from '../ui/primitives.jsx';

export default function AssistantChat() {
  const { state, c, step, upd, updC, askAi, t, lang } = useStore();
  ...
  return (
    <div data-noprint="1" style={{ background: 'var(--surface)', border: '1px solid var(--pri-border-2)', borderRadius: 10, marginTop: 24, overflow: 'hidden' }}>
      <div style={{ background: 'var(--pri)', padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ font: '700 13px Helvetica,Arial,sans-serif', color: 'var(--on-pri)' }}>{t('YZ Asistan', 'AI Assistant')} · {agentTitlesFor(lang)[step - 1]}</div>
        {messages.length ? (
          <HButton
          ...
          >{t('Sohbeti temizle', 'Clear chat')}</HButton>
        ) : null}
      </div>

      <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ font: '12.5px/1.55 Helvetica,Arial,sans-serif', color: 'var(--ink-4)' }}>{agentIntrosFor(lang)[step - 1]}</div>
        ...
        {state.aiBusy && !messages.some(m => m.live) ? (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', font: 'italic 12.5px Helvetica,Arial,sans-serif', color: 'var(--muted)' }}>
            <Spinner size={13} border={2} />{t('Asistan değerlendiriyor…', 'Assistant is evaluating…')}
          </div>
        ) : null}

        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <HButton
            onClick={() => askAi(t('Bu adımdaki girdilerimi metodolojiye göre değerlendir: eksikleri, karışıklıkları (problem / bulgu / kök neden ayrımı) ve iyileştirme önerilerini belirt.', 'Evaluate my inputs in this step against the methodology: point out gaps, confusions (problem / finding / root cause distinction), and improvement suggestions.'))}
          ...
          >{t('Bu adımdaki girdilerimi değerlendir', 'Evaluate my inputs in this step')}</HButton>
          <HButton
            onClick={() => askAi(t('Bu adım için mevcut verime dayanarak somut öneriler üret ve kendime ya da paydaşlarıma sormam gereken doğru soruları listele.', 'Based on my current data, generate concrete suggestions for this step and list the right questions I should ask myself or my stakeholders.'))}
            style={{ padding: '8px 13px', border: '1px solid var(--pri)', borderRadius: 20, background: 'var(--pri-soft)', color: 'var(--pri)', font: '600 12px Helvetica,Arial,sans-serif', cursor: 'pointer' }}
            hover={{ background: 'var(--pri-soft-hover)' }}
          >{t('Öneri ve doğru sorular üret', 'Generate suggestions and the right questions')}</HButton>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            placeholder={t('Asistana soru sorun — örn. Problem ifadem çözüm içeriyor mu?', 'Ask the assistant — e.g. Does my problem statement contain a solution?')}
