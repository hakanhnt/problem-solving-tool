...
const QUESTIONS = [
  ...
  "Hangi iş sürücülerinde (business driver) sapma olabilir?",
  'Hangi süreçlerde sorun olabilir? Hangi ana iş sürücüsü bozuk?',
  "Sorunlu KPI'ı hangi iş sürücülerindeki ya da süreçlerdeki bozukluk oluşturuyor?",
...
export default function Step2Drivers() {
  ...
  return (
    <div>
      ...
      <Card>
        <div style={{ ...S.cardTitle, margin: '0 0 4px' }}>Ana İş Sürücüleri</div>
        <div style={S.cardSub}>Sonucu oluşturan ana etkenleri ve ilgili süreçleri listeleyin.</div>
        <MethodBox margin="0 0 14px">İş sürücüsü haritalama (business driver mapping) — KPI'ı oluşturan ana etkenleri MECE (birbirini dışlayan, bütünü kapsayan) şekilde haritalayın. Şüpheleri masa başında değil, işi yapanlarla ve Gemba'da doğrulayın.</MethodBox>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div key={i} style={S.itemCard}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <textarea
                    ...
                    placeholder="İş sürücüsü / ana etken adı"
                    style={{ ...S.textarea, flex: 1, width: 'auto', font: '600 14px/1.45 Helvetica,Arial,sans-serif', minHeight: 44 }}
                  />
                  {aiReady ? <YZButton title={"YZ'den " + (i + 1) + ". iş sürücüsü için yardım al"} onClick={() => fieldHelp('Ana iş sürücüsü ' + (i + 1), (d.name || '') + (d.note ? ' — ' + d.note : ''))} /> : null}
          ...
          <AddButton onClick={() => updC(cc => cc.drivers.push({ name: '', note: '' }))}>+ İş sürücüsü ekle</AddButton>
        </div>
      </Card>

      {c.drivers.some(d => (d.name || '').trim()) ? (
        <Card>
          <div style={{ ...S.cardTitle, margin: '0 0 12px' }}>İş Sürücüsü Haritası <span style={{ font: '400 12px Helvetica,Arial,sans-serif', color: 'var(--muted)' }}>— görsel</span></div>
          ...
          <div style={{ font: '11px/1.5 Helvetica,Arial,sans-serif', color: 'var(--muted-2)', marginTop: 8 }}>İş sürücüsü ve alt bileşen girdilerinizden otomatik çizilir; Adım 3'te alt bileşen ekledikçe zenginleşir.</div>
        </Card>
      ) : null}
    </div>
  );
